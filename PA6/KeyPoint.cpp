...
vector<int> KeyPoint::findSource(vector<int> pixel, int j){
	// temp pixel to be returned
	vector<int> temp;

	// loop through key point vector and look for source
...
// returns destination pixel for a key point pixel
vector<int> KeyPoint::findDest(vector<int> pixel, int j){
	// temp pixel to be returned
	vector<int> temp;

	// loop through key point vector and look for destination
	for(unsigned int i = 0; i < keyPoints.size(); i++){
		if((pixel[0] == keyPoints[i][2*j]) && (pixel[1] == keyPoints[i][2*j+1])){
			temp.push_back(keyPoints[i][keyPoints[i].size()-2]);
			temp.push_back(keyPoints[i][keyPoints[i].size()-1]);
			return temp;
		}
	}
	return temp;
...
bool KeyPoint::isKeyPoint(vector<int> pixel, int j){
	for(unsigned int i = 0; i < keyPoints.size(); i++){
		if((pixel[0] == keyPoints[i][2*j]) && (pixel[1] == keyPoints[i][2*j+1])){
			return true;
		}

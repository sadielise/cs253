...
// linear morphing method
int ModifyImage::linearMorph(Image& image1, Image& image2, KeyPoint& kp){

	if((image1.getWidth() != image2.getWidth()) || (image1.getHeight() != image2.getHeight())){
		cerr << "ERROR: Linear Morph could not be executed because images do not have equal dimensions." << endl;
		return -1;
	}
	
	for(int j = 1; j <= kp.getN(); j++){
		Morpher morph(kp);
		int test = morph.morphImage(image1, image2, j);
		if(test < 0){
			cerr << "ERROR: Image could not be linearly morphed." << endl;
			return -1;
		}
	}

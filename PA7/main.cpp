...
TEST(KeyPoint9, kp9){
	...
	test.push_back(5);
	test.push_back(6);
	ASSERT_EQ(0, kp.isKeyPoint(test, 2));
}

// test if invalid key point returns -1
TEST(KeyPoint10, kp10){
	...
	test.push_back(3);
	test.push_back(4);
	ASSERT_EQ(-1, kp.isKeyPoint(test, 2));
}
...
TEST(KeyPoint11, kp11){
	...
	kp.readFile("kp7.txt");
	Mapping mp(kp);
	mp.findPixels(test, 2);	
	ASSERT_EQ(1,mp.getSourcePixel()[0]);
	ASSERT_EQ(2,mp.getSourcePixel()[1]);
}
...
TEST(KeyPoint12, kp12){
	...
	kp.readFile("kp7.txt");
	Mapping mp(kp);
	mp.findPixels(test, 1);
	ASSERT_EQ(21,mp.getDestPixel()[0]);
	ASSERT_EQ(24,mp.getDestPixel()[1]);
}

...
int Image::writeAsciiImage(string filename){
...
int Image::writeBinaryImage(string filename)
{
	// convert filename to a c string
	char cstring[100];	
	strcpy(cstring, filename.c_str());
	...
	// print pixels
	for(unsigned int i = 0; i < getPixels().size(); i++){
		unsigned char pixel = (unsigned char)getPixels()[i];
...
int Image::writeImage(string filename)
{
	...
	if(filename.substr(filename.length()-5,filename.length()-1) == ".pgma"){
		writeCheck = writeAsciiImage(filename);
	}
	else{
		writeCheck = writeBinaryImage(filename);
	}
